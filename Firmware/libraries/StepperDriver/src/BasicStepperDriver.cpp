...
void BasicStepperDriver::begin(float rpm, short microsteps){
	if ( xSemaphoreTake( xI2CSemaphore, ( TickType_t ) 50 ) == pdTRUE ){
		ioport.pinMode(dir_pin, OUTPUT);
		ioport.digitalWrite(dir_pin, HIGH);
		mem_dir_state = HIGH;
    ...
    if IS_CONNECTED(enable_pin){
		if ( xSemaphoreTake( xI2CSemaphore, ( TickType_t ) 50 ) == pdTRUE ){
			ioport.pinMode(enable_pin, OUTPUT);
			xSemaphoreGive( xI2CSemaphore );
		}
        disable();
    }
...
long BasicStepperDriver::nextAction(void){
    if (steps_remaining > 0){
         ...
         */
		if(dir_state != mem_dir_state)
		{
			if ( xSemaphoreTake( xI2CSemaphore, ( TickType_t ) 50 ) == pdTRUE ){
				ioport.digitalWrite(dir_pin, dir_state);
				mem_dir_state = dir_state;
				xSemaphoreGive( xI2CSemaphore );
			}
		}
...
void BasicStepperDriver::enable(void){
    if IS_CONNECTED(enable_pin){
		if ( xSemaphoreTake( xI2CSemaphore, ( TickType_t ) 50 ) == pdTRUE ){
			ioport.digitalWrite(enable_pin, enable_active_state);
			xSemaphoreGive( xI2CSemaphore );
		}
    };
    delayMicros(2);
...
void BasicStepperDriver::disable(void){
    if IS_CONNECTED(enable_pin){
		if ( xSemaphoreTake( xI2CSemaphore, ( TickType_t ) 50 ) == pdTRUE ){
			ioport.digitalWrite(enable_pin, (enable_active_state == HIGH) ? LOW : HIGH);
			xSemaphoreGive( xI2CSemaphore );
		}
    }
}
